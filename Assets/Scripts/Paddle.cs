...
public class Paddle : MonoBehaviour
{
    ...
    void AutoPlay()
    {
        ...
        //We create a new Vector 3 object to retrieve the paddle's current position. We also
        //create another Vector3 object to retrieve the ball's position
        Vector3 paddlePos = this.transform.position;
        Vector3 ballPos = ball.transform.position;

        //We use the Mathf.Clamp() method to comute the paddle's new x position, so it remains
        //within range. The paddle's position should be half a unit to the left of the ball's
        //position whenever this is possible, so the ball could always hot the center of the paddle.
        paddlePos.x = Mathf.Clamp(ballPos.x - 0.5f, minX, maxX);

        //Now that we have the paddle's new x position, we assign the world Vector3 object to
